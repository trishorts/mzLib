...
namespace Omics.Digestion
{
    public abstract class DigestionAgent
    {
            ...
            bool respectCleavageRequirements = false, IEnumerable<Modification> configuredModifications = null)
        {
            ...
            // An agent whose motif REQUIRES a modification has no site where that modification cannot be.
            // Removing those sites here, rather than dropping peptidoforms later, is what keeps product spans
            // and missed-cleavage counts correct: the read-through across a site that is not a site is simply
            // the ordinary product between the sites that remain, and needs no generation slack.
            ...
            // The second half of the promoting correction needs generation slack, for the opposite reason to
            // the filter above. The filter removes sites that can never be real. A site that SURVIVES the
            // filter is feasible but may still be UNOCCUPIED in a given peptidoform, and there the agent could
            // not have cut either -- so the read-through across it is a real product carrying one fewer real
            // missed cleavage than its span suggests. At the caller's budget it was never enumerated, which is
            // why the unglycosylated form of a glycoprotease substrate used to vanish instead of surviving
            // intact (truth-set STCE-08). DigestionProduct discounts the unoccupied sites back out of the
            // reported count and drops anything still over budget, so the slack cannot leak into the output as
            // products claiming more missed cleavages than were asked for.
            //
            // NOTE the slack is NOT MaxMods. That bound works for BLOCKING, where a blocked site carries a
            // modification, and fails for promoting, where an unjustified site is the complement: a feasible
            // site with none.
