...
namespace MassSpectrometry.Deconvolution.Parameters
{
    internal class FlashDeconvDeconvolutionParamters : DeconvolutionParameters
    {
        ...
        {
            public (double, double, double) tol; 
            public double min_mass; // Minimum mass (Da) to consider
            public double max_mass; // Maximum mass (Da) to consider
            public int min_charge; // Minimum charge state for MS1 spectra (can be negative for negative mode)
            public int max_charge; // Maximum charge state for MS1 spectra (can be negative for negative mode)
            public double min_mz; // If set to positive value, minimum m/z to deconvolve.
            public double max_mz; // If set to positive value, maximum m/z to deconvolve.
            public double min_rt; // If set to positive value, minimum RT to deconvolve.
            public double max_rt; // If set to positive value, maximum RT to deconvolve.
            public double isolation_window; // Default isolation window width. If the input mzML file does not contain isolation window width information, this width will be used.
            public (double, double, double) min_isotope_cosine; // Cosine similarity thresholds between avg. and observed isotope patterns for MS1, 2, ... (e.g., -min_isotope_cosine 0.8 0.6 to specify 0.8 and 0.6 for MS1 and MS2, respectively)
            public int allowed_isotope_error; // Allowed isotope index error for decoy and qvalue report. If it is set to 1, for example, +-1 isotope errors are not counted as false. Beta version.
            public double min_intensity; // Intensity threshold
        }

        private FlashDeconvSettings? _flashDeconvSettings;

        internal FlashDeconvSettings ToFlashSettings()
        {
            ...
            FlashDeconvSettings result = new FlashDeconvSettings
            {
                ...
                allowed_isotope_error = Allowed_isotope_error,
        ...
        public FlashDeconvDeconvolutionParamters((double ms1, double ms2, double ms3)? tolerance = null, double minMass = 50, double maxMass = 1000000, int minCharge = 1, int maxCharge = 100, 
            ...
            int allowedIsotopeError = 1, double minIntensity = 0.0, Polarity polarity = Polarity.Positive, AverageResidue? averageResidue = null) 
            : base(minCharge, maxCharge, polarity, averageResidue) 
        { 
